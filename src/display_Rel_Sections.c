...
int display_rel_section(fichierElf * MonfichierElf) {
  ...
  for (i = 0; i < MonfichierElf->nbRelSection; i++) {
	...
	if(ELF32_ST_TYPE(MonfichierElf->symTable[ELF32_R_SYM(MonfichierElf->RelSections[i].RelTable[j].r_info)].st_info)==3){//verifier si le type est section
		...
		//printf("\t%d\n",MonfichierElf->symTable[ELF32_R_SYM(MonfichierElf->RelSections[i].RelTable[j].r_info)].st_shndx);
		printf("\t%s\n",MonfichierElf->SectNames+MonfichierElf->secHeader[MonfichierElf->symTable[ELF32_R_SYM(MonfichierElf->RelSections[i].RelTable[j].r_info)].st_shndx].sh_name);

	}else{ //si le type n'est pas section
